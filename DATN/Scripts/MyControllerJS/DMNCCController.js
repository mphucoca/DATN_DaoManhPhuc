var app = angular.module('DMNCCApp', []);

app.controller('DMNCCController', function ($scope, $http) {
    ...
    $scope.DELETE_All_checked = function () {
        ...
        $scope.showConfirmModal('Bạn có chắc chắn muốn xóa các nhà cung cấp đã chọn?', function () {
            // Nếu người dùng đồng ý, tiến hành xóa các phiếu
            $http.post('/api/DMNCCAPI/DeleteAll', ds_checked)
                .then(function (response) {
    ...
    $scope.DELETE = function (item) {
        ...
        // Hiển thị modal xác nhận
        $scope.showConfirmModal('Bạn có chắc chắn muốn xóa nhà cung cấp này?', function () {
            // Nếu người dùng đồng ý, tiến hành xóa các phiếu
            $http.post('/api/DMNCCAPI/DeleteAll', ds_checked)
                .then(function (response) {
                    $scope.GET();  // reload danh sách
    ...
    $scope.SEARCH = function (isExpandSearch) {
        ...
        if (!isExpandSearch) {
            ...
            _data = {
                ...
                ten_ncc: $scope.search.ten_ncc,
                dia_chi: $scope.search.dia_chi,
                dien_thoai: $scope.search.dien_thoai,
                email: $scope.search.email,
                ghi_chu: $scope.search.ghi_chu

            };

        }
        $http.post('/api/DMNCCAPI/SEARCH', _data)
            .then(function (response) {
                if (response.data.success) {
    ...
    $scope.Save = function () {
        ...
        if ($scope.add_mode) {
            ...
            $http.post('/api/DMNCCAPI/SaveAdd', _data)
                .then(function (response) {
                    if (response.data.success) {
            ...
            // xử lý khi thêm mới bản ghi
            $http.put('/api/DMNCCAPI/SaveEdit', _data)
                .then(function (response) {
                    if (response.data.success) {
    ...
    $scope.ShowLichSu = function (item) {
        ...
        $http.get('/api/LOG2API/GetAuditLogByTable/')
            .then(function (response) {
    ...
    $scope.ShowLichSuCT = function (item) {
        $http.get('/api/LOG2API/GetAuditLogByTableCT/')
            .then(function (response) {
                $scope.historyLogs = response.data.filter(function (log) {
                    try {
                        // Parse primary_key_data để lấy ma_kho
                        const primaryKeyObj = JSON.parse(log.primary_key_data || '{}');
                        const logMaKho = primaryKeyObj.ma_ncc || null;

                        // So sánh ma_kho
                        const isSameMaKho = logMaKho === item.ma_ncc;
