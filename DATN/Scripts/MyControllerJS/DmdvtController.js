var app = angular.module('DmdvtApp', []);

app.controller('DmdvtController', function ($scope, $http) {
    ...
    $scope.DELETE_All_checked = function () {
        ...
        if (ds_checked.length === 0) {
            alert('Chưa chọn đơn vị tính nào để xóa');
            return;
        }

        // Hiển thị modal xác nhận
        $scope.showConfirmModal('Bạn có chắc chắn muốn xóa các đơn vị tính đã chọn?', function () {
            // Nếu người dùng đồng ý, tiến hành xóa các phiếu
            $http.post('/api/DmdvtAPI/DeleteAll', ds_checked)
                .then(function (response) {
    ...
    $scope.DELETE = function (item) {
        ...
        // Hiển thị modal xác nhận
        $scope.showConfirmModal('Bạn có chắc chắn muốn xóa đơn vị tính này?', function () {
            // Nếu người dùng đồng ý, tiến hành xóa các phiếu
            $http.post('/api/DmdvtAPI/DeleteAll', ds_checked)
                .then(function (response) {
                    $scope.GET();  // reload danh sách
    ...
    $scope.SEARCH = function (isExpandSearch) {
        ...
        if (!isExpandSearch) {
            ...
            _data = {
                keyword: $scope.search.keyword,
                ma_dvt: $scope.search.ma_dvt,
                ten_dvt: $scope.search.ten_dvt,
        ...
        $http.post('/api/DmdvtAPI/SEARCH', _data)
            .then(function (response) {
                if (response.data.success) {
    ...
    $scope.Save = function () {
        ...
        if ($scope.add_mode) {
            ...
            $http.post('/api/DmdvtAPI/SaveAdd', _data)
                .then(function (response) {
                    if (response.data.success) {
            ...
            // xử lý khi thêm mới bản ghi
            $http.put('/api/DmdvtAPI/SaveEdit', _data)
                .then(function (response) {
                    if (response.data.success) {
    ...
    $scope.ShowLichSu = function (item) {
        ...
        $http.get('/api/LOGAPI/GetAuditLogByTable/')
            .then(function (response) {
    ...
    $scope.ShowLichSuCT = function (item) {
        $http.get('/api/LOGAPI/GetAuditLogByTableCT/')
            .then(function (response) {
                $scope.historyLogs = response.data.filter(function (log) {
                    try {
                        // Parse primary_key_data để lấy ma_kho
                        const primaryKeyObj = JSON.parse(log.primary_key_data || '{}');
                        const logMaKho = primaryKeyObj.ma_dvt || null;

                        // So sánh ma_kho
                        const isSameMaKho = logMaKho === item.ma_dvt;
