...
app.controller('LoaiVatTuController', function ($scope, $http) {
            ...
            $http.post('/api/LoaiVatTuAPI/SaveAdd', _data)
                .then(function (response) {
                    if (response.data.success) {
                        ...
                        alert('Mã loại vật tư đã tồn tại');
                    }
                }).catch(function (error) {
                    alert('Đã có lỗi xảy ra liên hệ với quản trị viên'); 
                });
    ...
    $scope.ShowLichSu = function (item) {
        ...
        $http.get('/api/LOG5API/GetAuditLogByTable/')
            .then(function (response) {
    ...
    $scope.ShowLichSuCT = function (item) {
        $http.get('/api/LOG5API/GetAuditLogByTableCT/')
            .then(function (response) {
                $scope.historyLogs = response.data.filter(function (log) {
                    try {
                        // Parse primary_key_data để lấy ma_kho
                        const primaryKeyObj = JSON.parse(log.primary_key_data || '{}');
                        const logMaKho = primaryKeyObj.ma_loai_vt || null;

                        // So sánh ma_kho
                        const isSameMaKho = logMaKho === item.ma_loai_vt;
