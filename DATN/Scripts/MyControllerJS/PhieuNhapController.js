var app = angular.module('PhieuNhapApp', []);

app.controller('PhieuNhapController', function ($scope, $http) {
    ...
    $scope.ds_trangthai = [
        ...
        { ma_trangthai: 2, ten_trangthai: 'Đã hoàn thành' },
    ...
    $scope.showAddModal = function () {
        ...
        $scope.new_item = {
            ...
            trang_thai: 0 ,
            ma_ncc: $scope.ds_dmncc[0].ma_ncc,
            ngay_ct: new Date(),
            thue: 0,
            chietkhau: 0,
            tt_thanhtoan: 0,
        ...
        // khởi tạo các giá trị mặc định 
        $http.get('/api/phieunhap/tao_soct').then(function (response) {
            $scope.new_item.so_ct = response.data.so_ct;
        });
        $scope.getMaSoThueByMaNCC($scope.new_item.ma_ncc);
    ...
    $scope.DELETE_All_checked = function () {
        ...
        $scope.showConfirmModal('Bạn có chắc chắn muốn xóa các phiếu đã chọn?', function () {
            ...
            $http.post('/api/PhieuNhapAPI/DeleteAll', ds_checked)
                .then(function (response) {
  ...
  // Phần 3 các function cơ bản của hệ thống
    ...
    $scope.SEARCH = function (isExpandSearch) {
        ...
        $http.post('/api/PhieuNhapAPI/SEARCH', _data)
            .then(function (response) {
                if (response.data.success) {
    ...
    $scope.Save = function () {
        ...
        if ($scope.add_mode) {
            ...
            $http.post('/api/PhieuNhapAPI/SaveAdd', _data)
                .then(function (response) {
                    if (response.data.success) {
                        $scope.SaveAll();
            ...
            // xử lý khi thêm mới bản ghi
            $http.put('/api/PhieuNhapAPI/SaveEdit', _data)
                .then(function (response) {
                    if (response.data.success) {
                        $scope.SaveAll();
    ...
    $scope.Save_current_item = function () {
        ...
        $scope.current_item.ma_ncc = $scope.new_item.ma_ncc;
    ...
    $scope.getMaSoThueByMaNCC = function (ma_ncc) {
        if (ma_ncc) {
            $http.get('/api/DMNCCAPI/GetMaSoThue', { params: { ma_ncc: ma_ncc } })
                .then(function (response) {
                    if (response.data.success) {
    ...
    // Update item
    $scope.change_value_nhap = function () {
        $scope.current_item.thanh_tien = parseFloat($scope.current_item.so_luong_nhap) * parseFloat($scope.current_item.don_gia_nhap);
    }
 
    $scope.GET_ncc = function () {
        $http.get('/api/DMNCCAPI')
            .then(function (response) {
                $scope.ds_dmncc = response.data;
            })
            .catch(function (error) {
                alert('Đã có lỗi xảy ra, liên hệ với quản trị viên');
            });
    ...
    $scope.GET_CT = function (so_ct) {
        $http.get('/api/PhieuNhapAPI/GET_CT', { params: { so_ct: so_ct } })
            .then(function (response) {
    ...
    $scope.SaveAll = function () {
        ...
        $http.post('/api/PhieuNhapAPI/SaveAll', data)
            .then(function (response) {
                if (response.data.success) {
    ...
    $scope.UpdateTotal = function () {
        ...
        $scope.ds_ct_phieu.forEach(function (item) {
            const thanhTien = item.so_luong_nhap * item.don_gia_nhap;
            $scope.sum.so_luong += item.so_luong_nhap;
    ...
    $scope.ShowLichSu = function (item) {
        ...
        $http.get('/api/LOG6API/GetAuditLogByTable/')
            .then(function (response) {
    ...
    $scope.ShowLichSuCT = function (item) {
        $http.get('/api/LOG6API/GetAuditLogByTableCT/')
            .then(function (response) {
                $scope.historyLogs = response.data.filter(function (log) {
                    try {
                        // Parse primary_key_data để lấy ma_kho
                        const primaryKeyObj = JSON.parse(log.primary_key_data || '{}');
                        const logMaKho = primaryKeyObj.so_ct || null;

                        // So sánh ma_kho
                        const isSameMaKho = logMaKho === item.so_ct;
    ...
    ////<<< Phần duyệt trạng thái phiếu
    $scope.ChangeInfo = function (noidung, new_item) {
        if (!new_item || !new_item.so_ct) {
            alert("Không tìm thấy số chứng từ.");
            return;
        }

        if (noidung === 'HoanThanh') {
            // Xác nhận nhập kho đã xử lý riêng
            $http.post('/api/PhieuNhapAPI/XacNhanPhieuNhap?so_ct=' + new_item.so_ct)
                .then(function (response) {
                    $scope.new_item.trang_thai = 2;
                    alert("Thành công. Phiếu nhập đã được xác nhận và cập nhật tồn kho.");
                }, function (error) {
                  
                });
        } else {
            // Các trạng thái khác: Duyệt (1) hoặc Hủy (3)
            let new_status = (noidung === 'Duyet') ? 1 : (noidung === 'Huy' ? 3 : null);
            if (new_status !== null) {
                $http.post('/api/PhieuNhapAPI/CapNhatTrangThai?so_ct=' + new_item.so_ct + '&trang_thai=' + new_status)
                    .then(function (response) {
                        $scope.new_item.trang_thai = new_status;
