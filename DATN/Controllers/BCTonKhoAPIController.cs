...
{
    public class BCTonKhoAPIController : ApiController
    {
        private ApplicationDbContext db = new ApplicationDbContext();

        [HttpGet]
        [Route("api/BCTonKhoAPI/GetTonKho")]
        public IHttpActionResult GetTonKho()
        {
            var inputJson = "{\"ds_ma_vt\": \"\", \"ds_ma_kho\": \"\", \"group_yn\": true}";

            var sql = "SELECT * FROM bctontheokho(@p0::json)";
            var data = db.Database.SqlQuery<BCTonKho>(sql, inputJson).ToList();

            return Ok(data);
        }

        [HttpGet]
        [Route("api/BCTonKhoAPI/SearchTonKho")]
        public IHttpActionResult SearchTonKho(string ma_vt_list, string ma_kho_list)

        {
            // Gán các giá trị tìm kiếm khi export excel
        ...
        [HttpGet]
        [Route("api/BCTonKhoAPI/ExportTonKho")]
        ...
        "Mô tả ĐVT", "Tỷ lệ quy đổi", "Số lượng tồn", "Ngày cập nhật"
    };
                ...
                for (int i = 0; i < headers.Length; i++)
                {
                    ...
                    ws.Cells[1, i + 1].Style.Border.BorderAround(OfficeOpenXml.Style.ExcelBorderStyle.Thin); // Border tiêu đề
                }
                ...
                foreach (var item in data)
                {
                    ...
                    {
                        ws.Cells[row, 18].Value = item.ngay_cap_nhat.Value;
                        ws.Cells[row, 18].Style.Numberformat.Format = "dd/MM/yyyy";
                    }
                    ...
                    var dataRange = ws.Cells[row, 1, row, 18];
